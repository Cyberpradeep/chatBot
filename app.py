...
Override Format:
        if user ask line give me code or generate code or i want code for this or any other analogy that the above format steps are not required and any thing that related to generate code override the above format steps and give 
...
Note:
        while generating code in response use class name for code as code for all code generations and code expample and generate inside code inside a div and inside the div use <code> anchor tag
        and in some places you are using points or bullet point explanation inside <p> with * astriek for each point for that change them into list use <ul> and <li> tags
